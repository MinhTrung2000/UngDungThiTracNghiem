...
//  assert (id != -1); //debug
  if (id == -1) return Vector<string>();
  return rows[id];
}

void Table_t::clearAll() {
  if (tableView.empty()) return;
...
bool Table_t::mouseClick(int l, int t, Graphics& g) {
  ...
  if (btnFilter.mouseClick(l, t, g)) {
    if (backUpRows.empty()) return false;
    ...
    if (oldValue != newValue) {
      ...
      if (newValue == "") {
        ...
        for (int i = 0; i < rows.size(); ++i) {
          if (newValue.size() > rows[i][findOptionColumnIndex].size()) continue;
