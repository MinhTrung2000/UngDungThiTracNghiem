...
//	EventEngine *getPreProcess() { return preProcess; }
  ...
  void setMainFrame(Panel_t &panel) { mainFrame = &panel; }

  Rect_t *getMainFrame() { return mainFrame; }

	void run(bool clearScreen);

	//=============================
	static void setOnStopAllProcessFlag() { stopAllProcessFlag = true; }

	static EventEngine *getCurrentProcess() { return currentPocess; }

	static void setCurrentProcess(EventEngine *proc) { currentPocess = proc; }
};
