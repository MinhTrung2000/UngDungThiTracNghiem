...
public:
	...
	*/
	...
	void sort(int from, int to) {
	  if (empty()) return;
		if (! (0 <= from && from < sz && 0 <= to && to < sz && from < to)) return;

		int i = from;
		int j = to;
		T key = arr[(from + to) / 2];
		while (i < j) {
			while (i < to && arr[i] < key) ++i;
			while (j > from && arr[j] > key) --j;
			if (i <= j) {
				T tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
				++i;
				--j;
	...
	void sort(int from, int to, int (*f)(const T&, const T&)) {
	  if (empty()) return;
		...
		T key = arr[(from + to) / 2];
		while (i < j) {
			while (i < to && f(arr[i], key) == 1) ++i;
			while (j > from && f(arr[j], key) == -1 && arr[j] != key) --j;
			if (i <= j) {
				T tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
				++i;
				--j;
		...
		if (from < j) sort(from, j, f);
		if (i < to) sort(i, to, f);
	}

	Vector<T> unique() {
	  if (empty()) return Vector<T>();
    Vector<T> res;
    Vector<T> temp = *this;
    temp.sort();
    res.push_back(temp[0]);
    for (int i = 1; i < temp.size(); ) {
      if (temp[i] == res.back()) ++i;
      else {
