...
{
    if (lineId == textLines.size() - 1 || textLines[lineId + 1].empty())
        return;
    ...
    if (firstSpaceId == -1)
    {
        if (remaining != textLines[lineId + 1].size())
            return;
...
void TextArea_t::setValue(const string& value)
{
    ...
    for (int i = 0; i < value.size(); ++i)
    {
        int currY = currentPosition.Y - top - 1;
        //is full
        if (currY == textLines.size() - 1 && textLines[currY].size() == cord.X)
            return;
...
void TextArea_t::keyPress(int keycode, char ch, Graphics& g)
{
    ...
    {
        int currY = currentPosition.Y - top - 1;
        if (currentPosition.X == left + 1 + textLines[currY].size())
            return;
    ...
    case KEY_DOWN:
    {
        if (currentPosition.Y - top - 1 == textLines.size() - 1)
            return;
        if (textLines[currentPosition.Y - top].empty())
            return;
        ++currentPosition.Y;
        currentPosition.X = std::min((int) currentPosition.X, left + 1 + (int) textLines[currentPosition.Y - top - 1].size());
        break;
    ...
    {
        int currY = currentPosition.Y - top - 1;
        if (!textLines[textLines.size() -1].empty() || (currY == textLines.size() - 1))
            return;
...
COORD TextArea_t::getCurrentPosition()
{
    return currentPosition;
}

bool TextArea_t::IsCursorVisible()
{
    return true;
}
