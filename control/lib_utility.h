...
namespace StringUtility {
  ...
  //Having only alpha and space
	inline bool IsAlphaSpace(const string& input) {
		for (int i = 0; i < (int) input.size(); ++i)
			if (! isspace(input[i]) && ! isalpha(input[i])) return false;
		return true;
	}
	...
	inline bool HavingPunctChar(const string& input) {
		for (int i = 0; i < (int) input.size(); ++i)
			if (ispunct(input[i])) return true;
		return false;
	}
	...
	inline bool HavingSpace(const string& input) {
		for (int i = 0; i < (int) input.size(); ++i)
			if (isspace(input[i])) return true;
		return false;
	}
	...
	inline void Trim(string& input) {
		...
		int en = input.size() - 1;
		string correct = "";
		while (st <= en && input[st] == ' ') ++st;
		while (en >= st && input[en] == ' ') --en;
		if (st <= en) {
			for (int i = st; i <= en; ) {
				if (input[i] == ' ') {
					int cnt = 0;
					while (i + cnt <= en && input[i + cnt] == ' ') ++cnt;
					correct += ' ';
					i += cnt;
				} else {
					correct += input[i];
