...
public:
  ...
  DiemThi(const DiemThi& rhs) {
	...
	void setMaMon(const string& newMaMon) { maMonHoc = newMaMon; }

	string getMaMon() { return maMonHoc; }

	void setDiem(int newDiem) { diem = newDiem; }
	...
	Vector<CauHoiThi *> getDSCauHoiThi() { return dsCauHoi; }

	void setDSCautTraLoi(const Vector<char>& newDS) {
    dsCauTraLoi = newDS;
	}

	Vector<char> getDSCauTraLoi() { return dsCauTraLoi; }
