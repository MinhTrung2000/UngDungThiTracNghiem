...
public:
  ...
  : id(IdTrack++),
	...
	void setMaMon(const string& newMaMon) { maMon = newMaMon; }

	string getMaMon() const { return maMon; }

	void setCauHoi(const string& newCauHoi) { cauHoi = newCauHoi; }

	string getCauHoi() const { return cauHoi; }

	void setLuaChon(const Vector<string> &newLuaChon) { luaChon = newLuaChon; }

	Vector<string> getLuaChon() const { return luaChon; }

	void setDapAn(char newDapAn) { dapAn = newDapAn; }

	char getDapAn() const { return dapAn; }

	void setCanModifyFlag(bool flag) { canModifyFlag = flag; }

	bool getCanModifyFlag() const { return canModifyFlag; }
...
ostream& operator << (ostream& os, const CauHoiThi& rhs) {
  ...
  os << rhs.maMon << '\n';
  os << rhs.cauHoi << '\n';
  for (int i = 0; i < 4; ++i) os << rhs.luaChon[i] << '\n';
  os << rhs.dapAn << '\n';
